...
#include "caffe/layers/voxel_data2_layer.hpp"
...
VoxelData2Layer<Dtype>::~VoxelData2Layer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void VoxelData2Layer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const int border_line = this->layer_param_.voxel_data_param().border_line();
  ...
  VolumeDatum datum;
  long int id = shuffle_index_[lines_id_];
  if (!use_image){
   if (use_temporal_jitter){
	   //LOG(INFO) << "Data file: " << file_list_[id].c_str() << std::endl;
	   //LOG(INFO) << "Label: " << label_list_[id] << std::endl;
    CHECK(load_volume_datum_from_binary(file_list_[id].c_str(), label_list_[id], new_height, new_width, new_length, border_line, &datum));
   }
   else
    LOG(INFO) << "Not support" << std::endl;
  }
  else{
	LOG(INFO) << "Not support" << std::endl;
  }
...
template <typename Dtype>
void VoxelData2Layer<Dtype>::ShuffleClips() {
...
template <typename Dtype>
void VoxelData2Layer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  VoxelDataParameter voxel_data_param = this->layer_param_.voxel_data_param();
  ...
  const int border_line = voxel_data_param.border_line();
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    bool read_status;
    long int id = this->shuffle_index_[this->lines_id_];
    if (use_image){
		LOG(INFO) << "Not support" << std::endl;
    } else {
        if (use_temporal_jitter) {
			read_status = load_volume_datum_from_binary(this->file_list_[id].c_str(), this->label_list_[id], new_height, new_width, new_length, border_line, &datum);
        } else {
        	LOG(INFO) << "Not support" << std::endl;
        }
    }
...
INSTANTIATE_CLASS(VoxelData2Layer);
REGISTER_LAYER_CLASS(VoxelData2);

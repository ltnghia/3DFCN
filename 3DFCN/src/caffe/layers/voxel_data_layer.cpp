...
#include "caffe/layers/voxel_data_layer.hpp"
...
VoxelDataLayer<Dtype>::~VoxelDataLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void VoxelDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (!use_image){
   if (use_temporal_jitter){
    ...
    CHECK(load_volume_datum_from_binary(file_list_[id].c_str(), label_list_[id], new_height, new_width, new_length, &datum));
   }
   else
...
template <typename Dtype>
void VoxelDataLayer<Dtype>::ShuffleClips() {
...
template <typename Dtype>
void VoxelDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (use_image){
        ...
        if (use_temporal_jitter) {
			read_status = load_volume_datum_from_binary(this->file_list_[id].c_str(), this->label_list_[id], new_height, new_width, new_length, &datum);
        } else {
        	LOG(INFO) << "Not support" << std::endl;
        }
    }
...
INSTANTIATE_CLASS(VoxelDataLayer);
REGISTER_LAYER_CLASS(VoxelData);
